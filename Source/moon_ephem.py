...
class MoonEphem:
    def __init__(self, configuration, date_time, debug=False):
        ...
        self.location_time.lat = (radians(self.configuration.conf.getfloat(
            "Geographical Position", "latitude")))
        self.location_time.elevation = (self.configuration.conf.getfloat(
    ...
    def sun_direction(self):
        ...
        sinsd = (sin(self.sun_ra - self.ra) * cos(self.sun_de) /
                 sin(self.elongation))
        cossd = (sin(self.sun_de) * cos(self.de) -
                 cos(self.sun_de) * sin(self.de)
                 * cos(self.sun_ra - self.ra)) / sin(self.elongation)
    ...
    def update(self, date_time):
        ...
        self.sun_ra = s.ra
        self.sun_de = s.dec
        self.elongation = acos(sin(self.de) * sin(self.sun_de)
                               + cos(self.de) * cos(self.sun_de)
                               * cos(self.sun_ra - self.ra))
        self.pos_angle_sun = self.sun_direction()
        self.pos_angle_pole = self.pos_angle_sun + pi / 2.

        c = sqrt(s.earth_distance ** 2 + self.moon.earth_distance ** 2
                 - 2. * s.earth_distance * self.moon.earth_distance *
                 cos(self.elongation))
        sina = s.earth_distance * sin(self.elongation) / c
        cosa = (
            (s.earth_distance ** 2 - c ** 2 - self.moon.earth_distance ** 2) /
        ...
        if self.position_stored:
            if abs(current_time - self.stored_time) > 1.:
                self.rate_ra = (self.ra - self.stored_ra) / (
                    current_time - self.stored_time)
                self.rate_de = (self.de - self.stored_de) / (
                    current_time - self.stored_time)
                if self.configuration.protocol:
                    print str(date_time)[11:21], \
                        "Computing Moon speed (arc min./hour), RA: ", \
                        degrees(self.rate_ra) * 216000., \
                        ", DE: ", degrees(self.rate_de) * 216000., \
                        ", times (s): ", self.stored_time, current_time
    ...
    def compute_libration(self):
        ...
        ma = ephem.Equatorial(self.ra, self.de,
        ...
        omega_prime = atan2(-sin(cap_i) * sin(omega) / sin(i),
                            (cos(cap_i) * sin(epsilon) - sin(cap_i) * cos(
                                epsilon) * cos(omega)) / sin(i))

        self.pos_rot_north = atan2(-sin(i) * cos(omega_prime - self.ra),
                                   cos(self.de) * cos(i) - sin(self.de) * sin(
                                       i) * sin(omega_prime - self.ra))
        self.topocentric_lib_lat = asin(
            -sin(cap_i) * cos(me.lat) * sin(me.lon - omega) - cos(cap_i) * sin(
                me.lat))
        self.topocentric_lib_long = (atan2(
            cos(cap_i) * cos(me.lat) * sin(me.lon - omega) - sin(cap_i) * sin(
                me.lat),
...
if __name__ == "__main__":
    ...
    for i in range(30):
        ...
        print 'Moon RA: %s, DE: %s, Diameter: %s' % (me.ra, me.de,
                                                     ephem.degrees(
                                                         me.diameter))
        print 'Astrometric libration in Latitude: ', me.astrometric_lib_lat, \
            ", in longitude: ", me.astrometric_lib_long
        print "Topocentric libration in latitude: ", degrees(
            me.topocentric_lib_lat), ", in longitude: ", degrees(
            me.topocentric_lib_long)
        print "Position angle of North Pole: ", degrees(me.pos_rot_north)
        print 'Selenographic Co-Longitude: ', me.colong
        print 'Sunlit geographic longitudes between ', degrees(
            -me.colong), " and ", degrees(-me.colong) + 180.
        ...
        print ('Pos. angle pole (bright limb to the right): %s' %
               (ephem.degrees(me.pos_angle_pole)))
